<template>
  <el-card class="square-container">
    <el-form inline>
      <el-form-item>
        <el-input v-model="search" style="width: 200px" clearable placeholder="昵称、内容、地点搜索">
          <template #prefix>
      ...
      <el-form-item>
        <el-select v-model="hasCar" style="width: 150px" clearable placeholder="是否有车" @change="handleHasCarChange">
          <el-option v-for="item in carFilter" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-button type="info" @click="searchList">
        ...
        搜索</el-button
      >
      <el-button type="danger" v-if="isAuth(['ROOT', 'CARPOOL:DELETE'])" @click="onRemoveClick()">批量删除</el-button>
      <el-button type="success" v-if="isAuth(['ROOT', 'CARPOOL:UPDATE'])" @click="changeStatus(1)">批量上架</el-button>
      <el-button type="warning" v-if="isAuth(['ROOT', 'CARPOOL:UPDATE'])" @click="changeStatus(0)">批量下架</el-button>
    </el-form>
    <el-table
      :data="tableData"
      border
    ...
    >
      ...
      <el-table-column prop="status" label="是否有车" align="center" width="100">
        <template #default="{ row }">
          <el-tag v-if="row.status == 1" type="primary">有</el-tag>
          <el-tag v-else type="warning">无</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="origin" label="起点" width="200" align="center"></el-table-column>
      <el-table-column prop="destination" label="终点" width="200" align="center"></el-table-column>
      <el-table-column prop="goTime" label="出发时间" align="center" width="170">
        <template #default="{ row }">
          {{ row.goTime }}
        </template>
      </el-table-column>
      <el-table-column label="操作" fixed="right" align="center" width="300">
        <template #default="{ row }">
          ...
          <el-button
            :type="row.statusSec ? 'success' : 'warning'"
            size="small"
            v-if="isAuth(['ROOT', 'CARPOOL:UPDATE']) && multipleSelection.length == 0"
            @click="changeStatus(row)"
          >
            <template v-if="row.statusSec == 1"> 下架 </template>
            <template v-else> 上架 </template>
          </el-button>
          <el-button
            type="danger"
            size="small"
            v-if="isAuth(['ROOT', 'CARPOOL:DELETE']) && multipleSelection.length == 0"
...
import { getCarPoolList, deleteCarPools, updateStatus } from '@/api/carpool'
...
const getListData = async () => {
  ...
  const result = await getCarPoolList(param)
...
const changeStatus = async (row) => {
  ...
  if (row === 1) {
    ...
    row.statusSec = Number(!row.statusSec)
