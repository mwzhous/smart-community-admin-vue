  ...
  <el-card class="square-container">
    <el-form inline>
      ...
      <el-button type="info" @click="searchList">
        ...
        搜索</el-button
      >
      <el-button type="danger" v-if="isAuth(['ROOT', 'SQUARE:DELETE'])" @click="onRemoveClick()">批量删除</el-button>
      <el-button type="success" v-if="isAuth(['ROOT', 'SQUARE:UPDATE'])" @click="changeStatus(1)">批量上架</el-button>
      <el-button type="warning" v-if="isAuth(['ROOT', 'SQUARE:UPDATE'])" @click="changeStatus(0)">批量下架</el-button>
    </el-form>
    <el-table
      :data="tableData"
      border
      ...
      <el-table-column label="操作" fixed="right" align="center" width="300">
        <template #default="{ row }">
          ...
          <el-button
            ...
            size="small"
            v-if="isAuth(['ROOT', 'SQUARE:UPDATE']) && multipleSelection.length == 0"
          ...
          <el-button
            type="danger"
            size="small"
            v-if="isAuth(['ROOT', 'SQUARE:DELETE']) && multipleSelection.length == 0"
...
import { getSquareList, deleteSquares, updateStatus } from '@/api/square'
...
const getListData = async () => {
  ...
  const result = await getSquareList(param)
